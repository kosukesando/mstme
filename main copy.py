...
from mstme.mstme import MSTME, Area, SimulationConfig

os.environ["OPENBLAS_MAIN_FREE"] = "1"


def log(mstme: MSTME):
    with Path(mstme.dir_out).joinpath(rf"log.txt").open("w") as f:
        _output = ""
        _output += rf"Marginal threshold:\t{mstme.thr_pct_mar*100}%\n"
        for S in STM:
            vi = S.idx()
            _output += rf"\t{mstme.thr_mar[vi]}[{S.unit()}]"
            _output += rf"\tcount:{np.count_nonzero(mstme.is_e_mar[vi])}\n"
            _output += rf"\tGP params(xi,mu,sigma):{mstme.gp[vi].args}\n"
        _output += rf"Common threshold:\t{mstme.thr_pct_com*100}%\t{mstme.thr_com}\n"
        for S in STM:
            vi = S.idx()
            _output += rf"\tcount:{np.count_nonzero(mstme.is_e[vi])}\n"
            _output += rf"\tCM params(a,b,mu,sigma):{mstme.params_mean[vi]}\n"
        f.write(_output)
...
def load_data(region, depth):
    match region:
        ...
        case _:
            ...
            occur_freq = None
            dir_bathy = None
            raise (ValueError(rf"No region found with name {region}"))

    ds_path = Path(rf"./data/ds_filtered_{region}.dill")
    ds_track_path = Path(rf"./data/ds_track_{region}.dill")
    if ds_path.exists():
        ...
        for v in STM:
            ds_filtered[rf"STM_{v.key()}"] = ds_filtered[v.key()].max(dim="node")
            ds_filtered[rf"EXP_{v.key()}"] = (
                ds_filtered[v.key()] / ds_filtered[rf"STM_{v.key()}"]
            )
        print(ds_filtered)

        # dill Dataset
        with open(rf"./data/ds_filtered_{region}.dill", "wb") as fh:
...
if __name__ == "__main__":
    ...
    print(rf"SAVE:{SAVE},RECALCULATE:{RECALC},DRAW:{draw_fig},Subsample:{N_subsample}")

    ##### Load region MSTME object
    start_mstme = time.time()
    # Output stuff
    dt_string = datetime.now().strftime("%Y-%m-%d-%H%M")
    dir_out_region = rf"./output-new/{region}/"
    ...
    if path_dill_region.exists() and not RECALC:
        with path_dill_region.open("rb") as f:
            ...
            print(rf"MSTME Dill found for '{region}'!")
    else:
        print(rf"No dill found for '{region}' or recalculating!")
    ...
    for rf in [
        ...
        # Output stuff
        dt_string = datetime.now().strftime("%Y-%m-%d-%H%M")
        path_out_cluster = path_out_region.joinpath(rf"{rf}")
        ...
        if path_dill_cluster.exists() and not RECALC:
            with path_dill_cluster.open("rb") as f:
                ...
                print(rf"Cluster Dill found for '{rf}'!")
        else:
            print(rf"No dill found for '{rf}' or recalculating!")
        ...
        print(
            rf"MSTME object calculations and plots for {rf}, finished in {time.time()-start_mstme}"
        )
        ...
        for thr_pct_mar in [
            ...
            for thr_pct_com in [
                ...
                path_out_condition = (
                    ...
                    / rf"GP{round(thr_pct_mar*100)}%_CM{round(thr_pct_com*100)}%_{N_subsample}subsamples/"
                )
                ...
                path_dill_condition = path_out_condition.joinpath(
                    rf"mstme_condition.dill"
                )
                if path_dill_condition.exists() and not RECALC:
                    with path_dill_condition.open("rb") as f:
                        mstme_condition = dill.load(f)
                        print(
                            rf"MSTME Dill found for GP{round(thr_pct_mar*100)}%_CM{round(thr_pct_com*100)}%!"
                        )
                else:
                    print(
                        rf"MSTME dill not found or recalculating for GP{round(thr_pct_mar*100)}%_CM{round(thr_pct_com*100)}%!"
                    )
                ...
                path_mstme_ss_dill = path_out_condition.joinpath(
                    rf"mstme_ss_{N_subsample}_pool_{N_year_pool}.dill"
                )
                if path_mstme_ss_dill.exists():
                    ...
                    if tm_MSTME_ss.shape[0] != N_subsample:
                        warnings.warn(
                            rf"Sample count of {tm_MSTME_ss.shape[0]} for mstme_ss_dill does not match the input:{N_subsample} (GP{round(thr_pct_mar*100)}%_CM{round(thr_pct_com*100)}%)"
                        )
                else:
                    try:
                ...
                grapher_condition.draw_all(
                    [
                        ...
                        # "a+mub_Estimates",
                        # "Residuals",
                        # "Simulated_Conmul_vs_Back_Transformed_2",
                        # "Equivalent_fetch",
                        "STM_Histogram",
                        "STM_location",
                    ]
                ...
                print(
                    rf"Cluster object calculations and plots for {thr_pct_com},{thr_pct_mar},{rf} finished in {time.time()-start_condition}"
                )
