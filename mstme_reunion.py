...
if is_interactive():
    ...
    parser.add_argument("thr_gum", type=float, help="")
    parser.add_argument("thr_hs", type=float, help="")
    parser.add_argument("thr_u10", type=float, help="")
    parser.add_argument("--depth", type=float, help="", required=False, default=-100)
    parser.add_argument("--nbootstrap", type=int, help="", required=False, default=1)
    parser.add_argument(
...
idx_bootstrap = rng0.choice(num_events_total, (N_bootstrap, N_selevents), replace=False)
# tm_sample = np.zeros((N_bootstrap, num_vars, N_selevents, num_nodes))
tm_sample = []
tm_all = np.einsum(
    "ven,ve->ven",
    np.stack([ds_all.exp_h, ds_all.exp_u]),
    np.array([ds_all.stm_h, ds_all.stm_u]),
)
...
for bi in range(N_bootstrap):
    ...
    stme.kendall_tau_mv(
        stm_g, exp_ext, is_e, var_name, lonlat, dir_out=dir_out, draw_fig=False
    )
    # Estimate conmul parameters
    params_mean, residual = stme.estimate_conmul(
        stm_g, thr_gum, var_name, dir_out=dir_out, SEARCH=SEARCH_CTHR, draw_fig=True
    )
    # Sample from conmul model
    occur_prob = 1.04
    sample_full, ppf = stme.sample_stm(
        stm,
        stm_g,
        gp,
        f_hat_cdf,
    ...
    ax.scatter(stm_sample[0], stm_sample[1], edgecolors="red", s=30, facecolors="none")
    ax.set_xlabel(r"$H_s$")
    ax.set_ylabel(r"$U$")
    # ##################################################################################################################
    ...
    if SAVE:
        with open(f"{dir_out}/info.txt", "w") as f:
            ...
            f.write(f"Marginal Threshold:\t{thr_mar[0]}[m], {thr_mar[1]}[m/s]\n")
...
for i, ax in enumerate(axes.flatten()):
    ...
    for xi in range(res):
        for yi in range(res):
            _count = np.count_nonzero(
                np.logical_and(
                    tm_all[0, :, _idx_pos] > _x[xi],
                    tm_all[1, :, _idx_pos] > _y[yi],
                )
            )
            _z_mg[xi, yi] = _count
    ...
    _levels_original = [tm_all.shape[1] / (rp * occur_prob) for rp in return_periods]
    ax.contour(
        _x_mg,
        _y_mg,
        _z_mg.T,
...
for vi in range(num_vars):
    ...
    for ni in range(num_nodes):
        exp_ext[vi, :, ni] = exp[vi, :, ni] ** ((stm[vi] / thr_mar[vi]) ** kval[vi, ni])
    ...
    for ni in range(num_nodes):
        ...
        for k0 in np.linspace(-1, 1, 3):
            _optres = minimize(
                kendallpval,
                k0,
                args=(stm[vi, _mask] / thr_mar[vi], exp[vi, _mask, ni]),
                method="Powell",
            )
            if _optres.fun < _p:
                _k = _optres.x
...
for vi in range(num_vars):
    ...
    ax[vi].scatter(
        lonlat[:, 0],
        lonlat[:, 1],
        c=["red" if p < 0.05 else "black" for p in pval_ext[vi, :]],
        s=5,
    )
