        ...
        for thr_pct_com in [0.60, 0.65, 0.70, 0.75, 0.8]:
            ...
            if path_dill_mstme.exists() and not RECALC:
                with path_dill_mstme.open("rb") as f:
                    mstme = dill.load(f)
                    print(f"MSTME Dill found!")
            else:
                print(f"No dill found or recalculating!")
                mstme = MSTME(
                    area=area,
            ...
            for rf in [
                ...
                # Pickle MSTME object for faster redraws
                path_dill_cluster = path_out_cluster.joinpath(f"cluster_dill.dill")
                if path_dill_cluster.exists() and not RECALC:
                    with path_dill_cluster.open("rb") as f:
                        cluster = dill.load(f)
                        print(f"Cluster dill found!")
                else:
                    print(f"Cluster dill not found or recalculating!")
                    cluster_mask = mstme.get_region_filter(rf)
                    cluster = MSTME(
                        parent=mstme,
                        mask=cluster_mask,
