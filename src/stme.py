...
# Whether to use genpareto or laplace for normalization
# ndist = genextreme(0)
ndist = laplace
...
def get_region_filter(ds: xr.Dataset, region_filter: str = 'none', dir_out: str = None, draw_fig: bool = True) -> np.ndarray:
    """
    Returns an boolean mask ndarray for events based on STM location
    """
...
def _f_hat_cdf(pd_nrm, pd_ext, X):
    """
    The combined CDF described by H&T to combine empirical distribution with extremes model
    """
    X = np.asarray(X)
    scalar_input = False
    if X.ndim == 0:
...
def _f_hat_ppf(pd_nrm, pd_ext, _stm, X_uni):
    """
    The combined PPF described by H&T to combine empirical distribution with extremes model
    """
