...
class STM(enum.Enum):
    H = (0, "hs", r"$H_s$", r"$H_s^L$", "m")
    U = (1, "UV_10m", r"$U_{10}$", r"$U_{10}^L$", "m/s")
                ...
                try:
                    match method:
                        ...
                        case "ot_build_er":
                            distribution: (
                                ot.GeneralizedPareto
                            ) = ot.GeneralizedParetoFactory().buildMethodOfExponentialRegression(
                                _sample
                            )
                            _sp, _xp, _mp = distribution.getParameter()  # sigma,xi,mu
                        ...
                        case "ot_build_pwm":
                            distribution: (
                                ot.GeneralizedPareto
                            ) = ot.GeneralizedParetoFactory().buildMethodOfProbabilityWeightedMoments(
                                _sample
                            )
                            _sp, _xp, _mp = distribution.getParameter()  # sigma,xi,mu
...
def get_ss_pool(mstme: MSTME, num_ss, num_events_ss: int):
...
class MSTME:
    def __init__(
        ...
        **kwargs,
        ...
        self.gp, self.gp_params = genpar_estimation(self.stm, self.thr_mar)
        t3 = time.time()
    ...
    def sample_tm_PWE(self, idx_node: int, N_sample: int):
        ...
        _gp, _ = genpar_estimation(_tm, _thr_mar, method="MLE", N_gp=1)
        # print(_gp)
    ...
    def search_marginal(self, thr_start, thr_end, N_gp=100, N_THR=10):
        ...
        for ti, _thr in enumerate(thr_list):
            # print(_thr)
            _, _genpar_params = genpar_estimation(
    ...
    def subsample(
        ...
        _mask_ss = get_ss_pool(self, num_ss, _num_events_ss)
