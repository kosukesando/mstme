...
#########################################################
fig, ax = plt.subplots(len(par_name), num_vars, figsize=(8*num_vars, 6*len(par_name)))
...
#########################################################
fig, ax = plt.subplots(1, num_vars, sharey=True, figsize=(8*num_vars, 6), facecolor="white")
for vi in range(num_vars):
...
for vi in range(num_vars):
    ax[vi].set_xlabel("Longitude")
    ax[vi].set_ylabel("Latitude")
    im = ax[vi].scatter(lonlat[:, 0], lonlat[:, 1], s=5, c=tval[vi], cmap ='seismic', vmax=np.abs(tval[vi]).max(), vmin=-np.abs(tval[vi]).max())
...
for vi in range(num_vars):
    for vj in range(num_vars):
        ax[vi, vj].set_xlabel("Longitude")
        ax[vi, vj].set_ylabel("Latitude")
        im = ax[vi, vj].scatter(lonlat[:, 0], lonlat[:, 1], s=5, c=tval[vi,vj,:], cmap ='seismic', vmax=np.abs(tval[vi]).max(), vmin=-np.abs(tval[vi]).max())
